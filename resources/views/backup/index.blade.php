        ...
        <div class="backup-card-body">
            ...
            <div class="backup-meta">
                <span>
                    ...
                    Último respaldo: {{ $ultimoRespaldo ?? 'Nunca' }}
                </span>
                <span>
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg>
                    Tamaño actual: {{ $tamanoDb ?? '—' }}
                </span>
            </div>
            <form method="POST" action="{{ route('backup.generate') }}" style="display:inline;">
                @csrf
    ...
    <div class="backup-card backup-card--restore">
        ...
        <div class="backup-card-body">
            ...
            <form method="POST" action="{{ route('backup.restore') }}" enctype="multipart/form-data" id="restoreForm">
                @csrf
                {{-- Input oculto fuera del dropZone para evitar doble disparo --}}
                ...
                <tr>
                    ...
                    <td>
                        <div style="display:flex;gap:8px;align-items:center;">
                            {{-- Descargar --}}
                            <a href="{{ route('backup.download', $item['nombre']) }}" class="btn-hist-action btn-hist-download" title="Descargar">
                                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg>
                                Descargar
                            </a>
                            {{-- Restaurar desde historial --}}
                            <form method="POST" action="{{ route('backup.restoreSaved') }}" onsubmit="return confirm('¿Restaurar desde este respaldo? Los datos actuales serán reemplazados.')">
                                @csrf
